...
import 'package:fm_dictionary/data/services/ai_speech/ai_assistant/pronunciation_scorer.dart';
import 'package:fm_dictionary/data/services/ai_speech/ai_assistant/ai_assistant_service.dart';
import 'package:fm_dictionary/core/widgets/common/steak_celebration.dart';
...
class _StudyScreenState extends State<StudyScreen> {
  ...
  // Hàm bọc logic: Nhận vào hàm lưu Database và kết quả (Đúng/Sai)
  Future<void> _processAnswerWithStreakCheck(
    Future<void> Function() dbSaveAction, 
    bool isCorrect,
  ) async {
    // 1. Lấy Streak TRƯỚC KHI lưu
    final int streakBefore = _wordService.getCurrentStreak();

    // 2. Chạy hàm lưu tiến độ tương ứng với nút được bấm
    await dbSaveAction();

    // 3. Lấy Streak SAU KHI lưu
    final int streakAfter = _wordService.getCurrentStreak();

    // 4. Kiểm tra: Nếu Streak tăng lên -> Hôm nay vừa thắp lửa thành công!
    if (streakAfter > streakBefore && streakAfter > 0) {
      if (!mounted) return;
      
      // Push màn hình chúc mừng, dùng await để đợi người dùng bấm "Tiếp tục" rồi mới lật thẻ
      await Navigator.push(
        context,
        MaterialPageRoute(
          fullscreenDialog: true, // Mở dạng modal phủ kín màn hình
          builder: (context) => StreakCelebrationScreen(streakCount: streakAfter),
        ),
      );
    }

    // 5. Cuối cùng, gọi hàm lật thẻ hiện tại của bạn
    if (mounted) {
      _nextCard(isCorrect);
    }
  }
    ...
    return Row(
      ...
      children: [
        _ActionButton(
          ...
          onTap: () => _processAnswerWithStreakCheck(
            () => _wordService.updateProgress(currentWord.id, false), 
            false,
          ),
        ),
        ...
        _ActionButton(
          ...
          onTap: () => _processAnswerWithStreakCheck(
            () => _wordService.markAsLearned(currentWord.id), 
            true,
          ),
        ),
      ],
